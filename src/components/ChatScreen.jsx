...
import { uid, fmtTime, todayStr, getMeal } from '../lib/utils.js'

function IBtn({children,onClick,accent}) {
  return (
    <button onClick={onClick} style={{width:40,height:40,borderRadius:"50%",border:accent?"none":`1px solid ${T.border}`,background:accent?T.coral:T.bg,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,color:accent?"white":T.sub,boxShadow:accent?`0 2px 10px ${T.coral}40`:"none",touchAction:"manipulation"}}>
      {children}
    </button>
  )
}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:T.bg}}>
      ...
      {isMorning&&!checkedToday&&onboarded&&!sevCtx&&(
        <div style={{background:"#FFFDE7",borderBottom:"1px solid #FFF176",padding:"9px 16px",display:"flex",alignItems:"center",gap:10}}>
      ...
      {error&&(
        <div style={{background:"#FFF3F0",borderBottom:`1px solid ${T.coralMid}`,padding:"8px 16px"}}>
      ...
      {sevCtx&&<SeveritySlider value={sevVal} onChange={setSevVal} onConfirm={()=>sevCtx.onConfirm(sevVal)} label={sevCtx.label}/>}
        ...
        <div style={{background:T.card,borderTop:`1px solid ${T.border}`,padding:"8px 12px",paddingBottom:"max(10px,env(safe-area-inset-bottom))",display:"flex",alignItems:"flex-end",gap:8}}>
          ...
          <div style={{flex:1,background:T.bg,borderRadius:22,display:"flex",alignItems:"flex-end",padding:"9px 14px",border:`1px solid ${T.border}`,minHeight:40}}>
            <textarea value={input} onChange={e=>setInput(e.target.value)}
              ...
              placeholder="Message Patch..." rows={1}
              style={{flex:1,border:"none",outline:"none",background:"transparent",fontSize:16,fontFamily:"inherit",lineHeight:1.4,maxHeight:80,resize:"none",color:T.text}}/>
          </div>
