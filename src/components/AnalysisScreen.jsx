...
export function AnalysisScreen({foodLogs,symptomLogs,onBack}) {
  ...
  return (
    <div style={{position:"absolute",inset:0,background:T.bg,zIndex:200,display:"flex",flexDirection:"column",animation:"pslide 0.22s ease"}}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        <div style={{background:"#F2EDE6",position:"relative",overflow:"visible",minHeight:220}}>
          <img src={PATCH_BODY} alt="Patch" style={{position:"absolute",right:-10,bottom:0,width:220,objectFit:"contain",objectPosition:"bottom",display:"block",zIndex:1}}/>
          <div style={{position:"relative",zIndex:2,padding:"max(52px,calc(env(safe-area-inset-top) + 36px)) 0 28px 20px",maxWidth:"58%"}}>
            ...
            <p style={{margin:"0 0 16px",fontSize:12,color:"#8A7E76",lineHeight:1.65}}>{foodLogs.length} meals<br/>{symptomLogs.length} check-ins<br/>{new Set(foodLogs.map(l=>l.date)).size} days tracked</p>
            {!loading&&d.overall_confidence>0&&(
              <div style={{display:"inline-flex",alignItems:"center",gap:7,background:"white",borderRadius:20,padding:"6px 14px",boxShadow:"0 2px 8px rgba(0,0,0,0.08)"}}>
                ...
                <>
                  ...
                  {d.conditions.map((c,i)=>(
                    <div key={i} style={{background:T.card,borderRadius:14,padding:"14px 16px",marginBottom:8,border:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1}}><p style={{margin:"0 0 4px",fontSize:15,fontWeight:"700",color:T.text}}>{c.name}</p><p style={{margin:0,fontSize:13,color:T.sub,lineHeight:1.45}}>{c.note}</p></div>
        ...
        <div style={{padding:"0 16px 32px"}}>
          ...
          <button onClick={()=>setShowComposer(true)} style={{width:"100%",background:T.coral,color:"white",border:"none",borderRadius:13,padding:"14px",fontSize:14,fontWeight:"700",cursor:"pointer",boxShadow:`0 4px 16px ${T.coral}44`,display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:4,touchAction:"manipulation"}}>
      ...
      {showComposer&&<LetterComposer analysisData={d} foodLogs={foodLogs} symptomLogs={symptomLogs} onClose={()=>setShowComposer(false)}/>}
    </div>
  )
}
